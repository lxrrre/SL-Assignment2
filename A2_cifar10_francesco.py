...
class A2_cifar10:
    def __init__(self, solver_tag = 'lbfgs'):
        self.solver = solver_tag
        self.verbose = 1 # makes it so that more comments are printed while training
    ...
    def run_cross_validation(self, X_train_scaled, y_train_small, scoring='accuracy'):
        ...
        cv_model = LogisticRegressionCV(
            ...
            cv=self.folds,
            solver=self.solver,
            n_jobs=-1, #parallel computing for efficiency
            verbose=self.verbose,
            scoring=scoring
        )
        ...
        plt.ylabel('Accuracy' if scoring == 'accuracy' else 'Log Loss')
        plt.grid(True, which='both')
        plt.savefig('Cs_vs_scores.png')
        return cv_model
    ...
    def manual_log_loss_cv(self, X_train_scaled, y_train_small):
        ...
        plt.ylabel('Log Loss')
        plt.grid(True, which='both')
        plt.savefig('Cs_vs_log_loss_cv.png')
...
model_instance_saga = A2_cifar10(solver_tag='saga')
train_batches = [model_instance.unpickle(f"./data/cifar-10-batches-py/data_batch_{i}") for i in range(1,6)]
test_batch = model_instance.unpickle("./data/cifar-10-batches-py/test_batch")
meta_data = model_instance.unpickle("./data/cifar-10-batches-py/batches.meta")
...
lbgfs_time = time.time() - load_start
print("Training lbgfs base model took ", lbgfs_time)
saga_start_time = time.time()
saga_model = model_instance_saga.train_and_evaluate_model(1e-3, X_train_scaled, y_train_small, X_test_scaled, y_test_small)
saga_time = time.time() - saga_start_time
print("Training saga base model took ", saga_time)
...
final_model_1 = model_instance.make_final_model(best_C_log_loss, X_train_full, y_train)
